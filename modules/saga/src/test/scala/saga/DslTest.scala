...
import model.saga.{ActionCommand, Saga}
...
class DslTest extends WordSpec with Matchers {
  ...
  "action dsl" must {
    "create action dependency graph" in {
      ...
      val builder = SagaBuilder[String]()

      def create(a: String) = builder.addAction(rId(), s"actionType-$a", ActionCommand(rId(), s"Command-$a"))
      ...
      implicit val sagaState: Saga[String] = builder.build().right.get
