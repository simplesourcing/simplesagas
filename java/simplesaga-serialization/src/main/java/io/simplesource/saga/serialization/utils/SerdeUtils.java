...
public class SerdeUtils {
    public static <S extends SpecificRecord, V> Serde<V> iMap(Serde<S> sSerde, GenericMapper<V, S> mapper) {
        return iMap(sSerde, mapper::toGeneric, mapper::fromGeneric);
    }
    ...
    public static <S extends SpecificRecord, V> Serde<V> iMap(Serde<S> sSerde, Function<V, S> toGeneric, Function<S, V> fromGeneric) {
        Serializer<V> serializer = new Serializer<V>() {
            ...
            public byte[] serialize(String topic, V data) {
                return sSerde.serializer().serialize(topic, toGeneric.apply(data));
            }
